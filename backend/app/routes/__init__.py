...
def create_api_blueprint():
    ...
    def list_schedules():
        ...
        if not user_id:
            items = (
                Schedule.query.join(Category, isouter=True)
                .filter(
                    ((Category.admin_only == False) | (Schedule.category == None))
            ...
            if user.is_admin:
                ...
                include_user = False
        return jsonify([s.to_dict(include_user=include_user) for s in items])
